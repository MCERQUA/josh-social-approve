...
export default function TextOnlyPostCard({ post, onApprove, onReject, onUpdate, onDelete, onGenerateImage }: TextOnlyPostCardProps) {
  ...
  return (
    <div className={`w-full max-w-2xl rounded-lg border ${getStatusColor()} shadow-md hover:shadow-lg overflow-hidden transition-all duration-200 relative`}>
      {/* Delete Button */}
      {onDelete && (
        <button
          onClick={() => onDelete(post.id)}
          className="absolute top-2 right-2 z-10 w-7 h-7 bg-slate-700 hover:bg-red-600 text-slate-400 hover:text-white rounded-full flex items-center justify-center transition-all duration-200 shadow"
          title="Delete post"
        >
      ...
      <div className="p-4">
        <div className="flex items-start gap-3">
          {/* Small avatar */}
          <div className="w-10 h-10 rounded-full bg-gradient-to-br from-orange-500 to-orange-600 flex items-center justify-center text-white font-bold text-sm shadow flex-shrink-0">
            CCA
          </div>
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="flex items-center justify-between gap-2 mb-1">
              {isEditing ? (
                <input
                  ...
                  value={editedTitle}
                  onChange={(e) => setEditedTitle(e.target.value)}
                  className="flex-1 font-semibold text-white text-sm px-2 py-1 bg-slate-700 border border-blue-500 rounded focus:outline-none"
                  placeholder="Post title..."
                />
              ) : (
            ...
            {isEditing ? (
              <textarea
                ...
                onChange={(e) => setEditedContent(e.target.value)}
                rows={4}
                className="w-full text-slate-200 text-sm leading-relaxed px-2 py-1.5 bg-slate-700 border border-blue-500 rounded focus:outline-none resize-y"
                placeholder="Post content..."
              />
            ) : (
        ...
        {/* Generate Image Button (for approved posts) */}
        {post.approval?.status === 'approved' && onGenerateImage && (
          <button
            onClick={() => {
            ...
            className="mt-3 w-full px-4 py-2 bg-gradient-to-r from-purple-600 to-indigo-600 hover:from-purple-700 hover:to-indigo-700 disabled:from-slate-600 disabled:to-slate-600 text-white font-medium text-sm rounded-lg transition-all duration-200 flex items-center justify-center gap-2"
          >
            <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      {post.approval?.status === 'rejected' && post.approval.rejection_reason && (
        <div className="px-4 py-2 bg-red-500/10 border-t border-red-500/30">
          <p className="text-xs font-bold text-red-400">Rejected: <span className="font-normal text-red-300">{post.approval.rejection_reason}</span></p>
        </div>
      )}
        ...
        {isEditing ? (
          <div className="flex gap-2">
            <button
              onClick={handleSaveEdit}
              disabled={isSaving}
              className="flex-1 px-3 py-2 bg-blue-600 hover:bg-blue-700 disabled:bg-slate-600 text-white font-medium text-sm rounded-lg transition-all flex items-center justify-center gap-1.5"
            >
              <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M5 13l4 4L19 7" />
              </svg>
            ...
            <button
              onClick={handleCancelEdit}
              className="flex-1 px-3 py-2 bg-slate-600 hover:bg-slate-500 text-white font-medium text-sm rounded-lg transition-all flex items-center justify-center gap-1.5"
            >
              Cancel
            </button>
          </div>
          ...
          <div className="flex gap-2">
            <button
              onClick={() => onApprove(post.id)}
              disabled={post.approval?.status === 'approved'}
              className="flex-1 px-3 py-2 bg-green-600 hover:bg-green-700 disabled:bg-slate-700 disabled:text-slate-500 text-white font-medium text-sm rounded-lg transition-all flex items-center justify-center gap-1.5"
            >
              <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M5 13l4 4L19 7" />
              </svg>
            ...
            <button
              onClick={() => onReject(post.id)}
              disabled={post.approval?.status === 'rejected'}
              className="flex-1 px-3 py-2 bg-red-600 hover:bg-red-700 disabled:bg-slate-700 disabled:text-slate-500 text-white font-medium text-sm rounded-lg transition-all flex items-center justify-center gap-1.5"
            >
              <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M6 18L18 6M6 6l12 12" />
              </svg>
            ...
            <button
              onClick={() => setIsEditing(true)}
              className="px-3 py-2 bg-orange-600 hover:bg-orange-700 text-white font-medium text-sm rounded-lg transition-all flex items-center justify-center"
            >
              <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" />
              </svg>
