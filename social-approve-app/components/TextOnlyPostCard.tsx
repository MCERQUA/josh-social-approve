  ...
  return (
    <div className={`w-full max-w-2xl rounded-xl border-2 ${getStatusColor()} shadow-lg hover:shadow-xl overflow-hidden transition-all duration-200 relative`}>
      ...
      <div className="p-5 border-b border-slate-700">
        ...
        {/* Generate Image Button (for approved posts without images) */}
        {post.approval?.status === 'approved' && !hasImage && onGenerateImage && (
          <button
            onClick={() => onGenerateImage(post.id)}
            disabled={isGeneratingImage}
            className="mt-4 w-full px-5 py-3 bg-gradient-to-r from-purple-600 to-indigo-600 hover:from-purple-700 hover:to-indigo-700 disabled:from-slate-600 disabled:to-slate-600 text-white font-semibold rounded-lg transition-all duration-200 flex items-center justify-center gap-2 shadow-lg"
          >
