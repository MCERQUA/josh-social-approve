...
export async function POST(request: NextRequest) {
  try {
    ...
    try {
      ...
      // Use generateImages API with Imagen model for proper aspect ratio control
      console.log('[ImageGen] Calling Imagen 3 via generateImages API with 1:1 aspect ratio...');
      ...
      try {
        const result = await genAI.models.generateImages({
          ...
          config: {
            numberOfImages: 1,
            aspectRatio: '1:1',  // Native 1:1 square generation
        ...
        console.log('[ImageGen] Got 1:1 square image from Imagen');
      } catch (sdkError) {
        const errorMessage = sdkError instanceof Error ? sdkError.message : String(sdkError);
        console.error('[Imagen SDK] Error:', errorMessage);

        // Parse specific error types
        if (errorMessage.includes('authentication') || errorMessage.includes('API key')) {
          throw new Error('Imagen authentication failed - check API key');
        }
        if (errorMessage.includes('rate limit') || errorMessage.includes('quota')) {
          throw new Error('Imagen rate limit exceeded - try again later');
        }
