...
export default function BrandPage() {
  ...
  const fetchBrandAndStats = async () => {
    try {
      // Fetch tenant info which includes brands
      const tenantRes = await fetch('/api/tenant');
      if (!tenantRes.ok) throw new Error('Failed to fetch tenant');
      const tenantData = await tenantRes.json();

      // Find the brand by slug (case-insensitive)
    ...
    return (
      <div className="min-h-screen flex items-center justify-center">
        <div className="text-center">
          <h1 className="text-2xl font-semibold text-white mb-2">Brand Not Found</h1>
          <p className="text-slate-400 mb-4">{error || `No brand found with slug "${brandSlug}"`}</p>
  ...
  const colors = colorMap[brand.color] || colorMap.cyan;
  const brandPath = `/social/${brand.slug.toUpperCase()}`;
  ...
  const modules = [
    ...
    },
    {
      ...
      href: `${brandPath}/schedule`,
      available: true
    },
    {
      id: 'content-library',
      name: 'Content Library',
      description: 'Browse all images and videos',
      icon: (
        <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z" />
        </svg>
      ),
      href: `${brandPath}/content-library`,
      available: true
    },
    {
      id: 'analytics',
        ...
        <div className="max-w-7xl mx-auto px-6 py-8">
          ...
          <div className="flex items-center gap-4">
            <div className={`w-12 h-12 rounded-lg ${colors.bg} border ${colors.border} flex items-center justify-center flex-shrink-0`}>
              <span className={`text-lg font-semibold ${colors.text}`}>{brand.short_name || brand.slug.toUpperCase()}</span>
            </div>
            <div>
              <h1 className="text-3xl font-semibold text-white">{brand.name}</h1>
        ...
        <section className="mb-10">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            <div className="bg-slate-800/50 backdrop-blur-sm rounded-lg p-5 border border-slate-700/50 hover:border-emerald-500/50 transition-colors">
              ...
              <p className="text-3xl font-semibold text-emerald-400">{stats.approved}</p>
            </div>
            <div className="bg-slate-800/50 backdrop-blur-sm rounded-lg p-5 border border-slate-700/50 hover:border-rose-500/50 transition-colors">
              ...
              <p className="text-3xl font-semibold text-rose-400">{stats.rejected}</p>
            </div>
          </div>
        </section>
