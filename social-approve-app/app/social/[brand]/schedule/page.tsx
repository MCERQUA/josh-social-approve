...
export default function SchedulePage() {
  ...
  const fetchInstances = useCallback(async () => {
    ...
    try {
      ...
      const response = await fetch(
        `/api/schedules/instances?brand=${brandSlug}&start=${start.toISOString()}&end=${end.toISOString()}`
      );
  ...
  }, [currentDate, brandSlug]);

  const fetchReadyPosts = useCallback(async () => {
    if (!brandSlug) return;
    try {
      const response = await fetch(`/api/schedule/ready?brand=${brandSlug}`);
