...
async function runMigration() {
  ...
  console.log('Running migration: Add brand image_config column...\n');

  try {
    // Add image_config column
    console.log('1. Adding image_config JSONB column to brands...');
    await sql`
    ...
    // Update CCA brand
    console.log('2. Setting CCA image config...');
    await sql`
      UPDATE brands SET image_config = ${JSON.stringify({
        primaryColor: "#F97316",
    ...
    // Update ICA brand
    console.log('3. Setting ICA image config...');
    await sql`
      UPDATE brands SET image_config = ${JSON.stringify({
        primaryColor: "#00CED1",
    ...
    // Show results
    console.log('\n4. Verifying brand configs:');
    const brands = await sql`
      SELECT slug, name,
             image_config->>'primaryColor' as primary_color,
             image_config->>'logoPath' as logo_path,
             image_config->>'industry' as industry
      FROM brands
